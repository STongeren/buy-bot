...
def deploy_to_vps():
    ...
    # Handle processed_contracts.txt specially
    if 'processed_contracts.txt' in missing_files:
        missing_files.remove('processed_contracts.txt')
        # Create empty processed_contracts.txt if it doesn't exist
        if not os.path.exists('processed_contracts.txt'):
            with open('processed_contracts.txt', 'w') as f:
                f.write('# Processed contract addresses\n')
            print("Created processed_contracts.txt")
